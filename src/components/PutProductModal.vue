    ...
    <div v-if="show" class="modal-shadow" @click.self="closeModal">
      <div class="modal">
        ...
        <slot name="body">
          ...
          <div>
            <select required class="modal-content__input" name="category" v-model="categoryName">
        ...
        <slot name="footer">
          <div class="modal-footer">
            <button class="modal-footer__button" @click="editProduct">
                РЕДАКТИРОВАТЬ
            </button>
          </div>
        </slot>
...
export default {
  ...
  methods: {
    ...
    const productMainImageInput = this.$refs.productMainImageInput;
    if (productMainImageInput.files.length > 0) {
        data.append('main_image', productMainImageInput.files[0]);
    }
    data.append('name', this.productName);
    data.append('slug', this.productSlug);
    data.append('price', this.productPrice);
    data.append('category', this.categoryName);
  ...
  axios.put(`api/v1/admin/products/${this.productId}/`, data)
    .then(response => {
